...
async def test_read_user(async_db):
    ...
    async with async_db.begin():
        ...
        user = UserModel(username="testuser", password=hashed_password, dob="1990-01-01", country="Nowhere")
        async_db.add(user)
...
async def test_update_user(async_db):
    ...
    async with async_db.begin():
        hashed_password = bcrypt.hashpw("testpassword".encode("utf-8"), bcrypt.gensalt()).decode("utf-8")
        user = UserModel(username="olduser", password=hashed_password, dob="1990-01-01", country="Nowhere")
        async_db.add(user)
...
async def test_delete_user(async_db):
    ...
    async with async_db.begin():
        hashed_password = bcrypt.hashpw("testpassword".encode("utf-8"), bcrypt.gensalt()).decode("utf-8")
        user = UserModel(username="deletableuser", password=hashed_password, dob="1990-01-01", country="Nowhere")
        async_db.add(user)
        await async_db.flush()
        user_id = user.id
